...
{#key isOn}
    <div on:mouseenter={entered} on:mouseleave={left} class="group text-sm text-gray-400 p-4 pl-5 py-2 rounded-md m-2 h-full relative">
        {#if !song}
        ...
        <a class="px-3 py-1 mt-2 absolute top-100 right-0 rounded-md items-center {isOn ? "opacity-1" : "opacity-0"} bg-gray-100 hover:bg-gray-700 dark:hover:bg-white dark:hover:text-gray-900 hover:text-white dark:bg-gray-50/10 text-sm transition-all delay-75" aria-label="Dashboard" rel="noopener" href="/dashboard">
            Top tracks 
        </a>
    </div>
{/key}
